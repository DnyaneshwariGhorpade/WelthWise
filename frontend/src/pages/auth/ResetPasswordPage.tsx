...
import { Shield, Eye, EyeOff, AlertCircle, CheckCircle2, ArrowLeft } from 'lucide-react';
...
export default function ResetPasswordPage() {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center px-4">
        <div className="w-full max-w-md bg-white rounded-2xl shadow-sm border border-slate-200 p-8 text-center">
          ...
          <p className="mt-2 text-sm text-slate-500">This password reset link is invalid or missing a token.</p>
          <Link to="/forgot-password" className="mt-6 inline-block text-sm text-emerald-600 hover:text-emerald-700 font-medium">
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setError(axiosErr.response?.data?.error?.message || 'Reset failed. The link may have expired.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center px-4 py-12">
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
        ...
        <p className="mt-2 text-sm text-slate-500 text-center">Choose a strong password for your account</p>
        ...
        <p className="mt-6 text-center text-sm text-slate-500">
          <Link to="/login" className="inline-flex items-center gap-1 text-emerald-600 hover:text-emerald-700 font-medium">
